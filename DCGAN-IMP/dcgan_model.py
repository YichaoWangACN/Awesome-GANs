...
class DCGAN:
    ...
    def __init__(self, s, batch_size=64, height=64, width=64, channel=3,
        ...
        # Output Settings
        :param sample_num: the number of sample images, default 16
        ...
        # init image size in 1st lay of generator (4 layer)
        self.init_conv_height  =  height // (2 ** 4)
        self.init_conv_width  =  width // (2 ** 4)
    ...
    def generator(self, z, reuse=None, is_train=True):
        with tf.variable_scope('generator', reuse=reuse):
            x = t.dense(z, self.gf_dim * 8 * self.init_conv_height * self.init_conv_width, name='gen-fc-1')

            x = tf.reshape(x, [-1, self.init_conv_height, self.init_conv_width, self.gf_dim * 8])
    ...
    def bulid_dcgan(self):
        ...
        """
        d_real_loss = t.sce_loss(
            d_real, tf.ones_like(d_real) * self.label_smoothing)
        d_fake_loss = t.sce_loss(d_fake, tf.zeros_like(d_fake))
        self.d_loss = d_real_loss + d_fake_loss
        self.g_loss = t.sce_loss(d_fake, tf.ones_like(d_fake) * self.label_smoothing)
